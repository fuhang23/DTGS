...
class ModelParams(ParamGroup): 
    def __init__(self, parser, source_path,sentinel=False):
        self.sh_degree = 3
...
class OptimizationParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.iterations = 3_500
        self.position_lr_init = 0.00016
        self.position_lr_final = 0.0000016
        self.position_lr_delay_mult = 0.01
        self.position_lr_max_steps = 3_500
...
def get_combined_args(parser : ArgumentParser):
    # sys.argv获取命令行参数列表，并排除第一个元素（通常是脚本的文件名）
    ...
    try:
        cfgfilepath = os.path.join(args_cmdline.model_path, "cfg_args") # 配置文件的路径
        print("Looking for config file in", cfgfilepath)
        with open(cfgfilepath) as cfg_file:
            print("Config file found: {}".format(cfgfilepath))
            cfgfile_string = cfg_file.read() # 读取配置文件
    except TypeError:
        print("Config file not found at")
        pass
    args_cfgfile = eval(cfgfile_string)

    merged_dict = vars(args_cfgfile).copy() # 将读取到的参数转为字典的形式
