...
class Dthermal_Dataset(object):
    def __init__(
        ...
        self.train_idxs = [i for i in range(n_frames) if i % test_every != 0]
        self.test_idxs = [i for i in range(n_frames) if i % test_every == 0]
        self.video_idxs = [i for i in range(n_frames)]

        self.maxtime = 1.0

    def parse_colmap_intrinsics(self, cam_intrinsics):

        camera_id = next(iter(cam_intrinsics.keys()))
        cam = cam_intrinsics[camera_id]
    ...
    def qvec2rotmat(self, qvec):
        """Convert a COLMAP quaternion to a rotation matrix."""
        qw, qx, qy, qz = qvec
        R = np.array([
    ...
    def load_meta(self):
        """Load camera parameters in COLMAP format."""
        try:
            cameras_extrinsic_file = os.path.join(self.root_dir, "sparse/0", "images.bin")
            cameras_intrinsic_file = os.path.join(self.root_dir, "sparse/0", "cameras.bin")
            cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)  
            cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
        except:
            cameras_extrinsic_file = os.path.join(self.root_dir, "sparse/0", "images.txt")
            cameras_intrinsic_file = os.path.join(self.root_dir, "sparse/0", "cameras.txt")
            cam_extrinsics = read_extrinsics_text(cameras_extrinsic_file)
            cam_intrinsics = read_intrinsics_text(cameras_intrinsic_file)

        
        cam_extrinsic_list = list(cam_extrinsics.values())
        self.cam_extrinsic_sorted = sorted(cam_extrinsic_list, key=lambda x: x.name)

        focal_x, focal_y, cx, cy, width, height = self.parse_colmap_intrinsics(cam_intrinsics)
        self.img_wh = (
            int(width ),
            int(height ),
        )
        self.focal = (focal_x, focal_y)
        # get paths of images, depths, masks, etc.
        agg_fn = lambda filetype: sorted(glob.glob(os.path.join(self.root_dir, filetype, "*.png")))
        agg_depth = lambda filetype: sorted(glob.glob(os.path.join(self.root_dir, filetype, "*.npy")))
        self.image_paths = agg_fn("images")
        self.depth_paths = agg_depth("depth_dam")
        self.masks_paths = agg_fn("masks")

        self.num = len(self.image_paths)

        self.K = np.array([[focal_x, 0, cx],
                           [0, focal_y, cy],
    ...
    def format_infos(self, split):
        ...
        for idx in tqdm(idxs):
            ...
            # times
            time = idx / max(self.num - 1, 1)
            # poses
            
            per_extrinsic = self.cam_extrinsic_sorted[idx]
            R, T = self.qvec2rotmat(per_extrinsic.qvec), per_extrinsic.tvec
            self.R = R
            self.T = T
            # fov
            FovX = focal2fov(self.focal[0], self.img_wh[0])
            FovY = focal2fov(self.focal[1], self.img_wh[1])
            cameras.append(Camera(colmap_id=idx, R=R, T=T, FoVx=FovX, FoVy=FovY, image=image, depth=depth, mask=mask,
                                  gt_alpha_mask=None,
    ...
    def search_pts_colors_with_motion(self, ref_pts, ref_color, ref_mask, ref_c2w):
        ...
        interval = 1
        if len(self.image_paths) > 150:  # in case long sequence
            interval = 2
        for j in range(1, len(self.image_paths), interval):
