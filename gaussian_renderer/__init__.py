...
def render_flow(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    means3D_deform, scales_deform, rotations_deform, opacity_deform, atmos_attenuation, atmos_bias = pc.deformation(means3D[deformation_point], scales[deformation_point],
    ...
    # ARTM Eq. (5)(6): attenuate Gaussian radiance and add a learned thermal bias.
    if colors_precomp is not None:
        colors_precomp = colors_precomp * atmos_attenuation + atmos_bias
    elif shs is not None:
        shs = shs * atmos_attenuation.unsqueeze(2) + atmos_bias.unsqueeze(2)
